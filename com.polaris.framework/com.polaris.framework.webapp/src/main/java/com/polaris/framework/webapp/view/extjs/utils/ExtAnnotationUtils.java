package com.polaris.framework.webapp.view.extjs.utils;
...
import com.polaris.framework.webapp.view.extjs.jsp.BaseTagSupport;
...
public final class ExtAnnotationUtils
{
	...
	public static Method findDockedMethod(Object target, Object dockedChild)
	{
		...
		{
			throw new IllegalArgumentException("target or dockedType is null! target=" + target + ",dockedChild=" + dockedChild);
		}
		...
		for (Method method : methods)
		{
			if (method.isAnnotationPresent(TagDocked.class))
			{
				...
				{
					// 方法参数不唯一,是个错误的情况
					log.warn("Target:" + target.getClass() + " method:" + method.getName() + ", params number is not 1!");
					continue;
				}
